...
import urllib
from jauth.models import Userinfo
from .utils import GenerateRndFilename
from jcsbms.image_processing import create_outline,should_create_thumbnail, compress_quality
#from album.models import add_image
...
def save_upload_file(PostFile,FilePath):
    ...
    if PostFile.content_type in mime2type:
        compress_quality(PostFile,FilePath)
...
def list_files(request):
    ...
    files=[]
    root_path=os.path.join(USettings.gSettings.MEDIA_ROOT,uploadpath+listpath[action]).replace("\\","/")
    files=get_files(root_path,root_path,allowFiles[action],userinfo)
...
def get_files(root_path,cur_path, allow_types=[],userinfo=None):
    files = []
    items = os.listdir(cur_path)
    for item in items:
        item=unicode(item)
        item_name = os.path.splitext(item)[0]
        if item_name[-7:] == u"outline":
            continue;
        item_fullname = os.path.join(root_path,cur_path, item).replace("\\", "/")
        if os.path.isdir(item_fullname):
            files.extend(get_files(root_path,item_fullname, allow_types,userinfo))
        else:
            ext = os.path.splitext(item_fullname)[1]
            is_allow_list= (len(allow_types)==0) or (ext in allow_types)
            if is_allow_list:
                files.append({
                    "url":urllib.basejoin("/static/upfiles/"+userinfo.user_dir+"/" ,os.path.join(os.path.relpath(cur_path,root_path),item).replace("\\","/" )),
...
def UploadFile(request):
    ...
    if state=="SUCCESS":
        ...
        if action=="uploadscrawl" or action=="uploadimage":
            saved_path = "upfiles/"+userinfo.user_dir+"/"+OutputFile
            if should_create_thumbnail(saved_path):#同步制作缩略图
                create_outline(saved_path)
            #add_image(urllib.basejoin("upfiles/"+userinfo.user_dir+"/" , OutputFile),userinfo)#图片要加到相册数据中
    #返回数据
    return_info = {
        'url': urllib.basejoin("/static/upfiles/"+userinfo.user_dir+"/" , OutputFile) ,                # 保存后的文件名称
...
def catcher_remote_image(request):
    ...
    for remote_url in remote_urls:
        ...
        if remote_original_ext  in allow_type:
            ...
            if state == "SUCCESS":
                saved_path = "upfiles/"+userinfo.user_dir+"/"+o_file
                if should_create_thumbnail(saved_path):#同步制作缩略图
                    create_outline(saved_path)
                #add_image(urllib.basejoin("upfiles/"+userinfo.user_dir+"/" , o_file),userinfo)#图片要加到相册数据中
                
            catcher_infos.append({
                "state":state,
                "url":urllib.basejoin("/static/upfiles/"+userinfo.user_dir+"/" , o_file),
...
def get_output_path(request,path_format,path_format_var):
    ...
    OutputPath=os.path.join(USettings.gSettings.MEDIA_ROOT,uploadpath + OutputPath)
    if not OutputFile:#如果OutputFile为空说明传入的OutputPathFormat没有包含文件名，因此需要用默认的文件名
        ...
        OutputFile = GenerateRndFilename(OutputFile)
